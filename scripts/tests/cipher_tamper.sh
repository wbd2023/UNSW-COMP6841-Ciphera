...
RELAY_LOG="/tmp/ciphera-relay-test-cipher-tamper.log"
...
# Fetch and tamper ciphertext
ENV="$(curl -s "${RELAY_URL}/msg/${BOB_USER}" | jq '.[0]')"
CIPH="$(jq -r '.cipher' <<<"${ENV}")"
FIRST_CHAR="${CIPH:0:1}"
REPLACEMENT="A"
if [[ "${FIRST_CHAR}" == "A" ]]; then
  REPLACEMENT="B"
fi
TAMPERED="${REPLACEMENT}${CIPH:1}"

echo "Tampering ciphertext: original first byte='${FIRST_CHAR}', replacement='${REPLACEMENT}'"

jq --arg c "${TAMPERED}" '.cipher = $c' <<<"${ENV}" \
  | curl -sS -X POST -H 'Content-Type: application/json' -d @- \
    "${RELAY_URL}/msg/${BOB_USER}" >/dev/null

# Bob tries to receive: should fail AEAD
set +e
"${CIPHERA_BIN}" recv \
  --home       "${BOB_HOME}" \
  --username   "${BOB_USER}" \
...
if [[ ${RC} -eq 0 ]]; then
  echo "[-] Tampered ciphertext was accepted!"
  cat /tmp/t2.out
  exit 1
fi

echo "[+] Tampered ciphertext rejected (AEAD integrity intact)."
