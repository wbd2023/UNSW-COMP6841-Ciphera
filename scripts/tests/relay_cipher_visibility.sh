...
ALICE_HOME="/tmp/alice-ciphera-t1-alice"
BOB_HOME="/tmp/bob-ciphera-t1-bob"
ALICE_USER="alice"
BOB_USER="bob"
...
RELAY_LOG="/tmp/ciphera-relay-t1.log"
...
# Send a message
"${CIPHERA_BIN}" send --home "${ALICE_HOME}" --username "${ALICE_USER}" --relay "${RELAY_URL}" --passphrase "${ALICE_PASS}" "${BOB_USER}" "${SECRET_MSG}"

# Inspect relay queue for Bob
RAW="$(curl -s "${RELAY_URL}/msg/${BOB_USER}")"
echo "${RAW}" | jq '.[0] | {from,to,has_header:(.header|has("N")), has_cipher:has("cipher") }'

# Assert: plaintext is NOT present on the relay
if echo "${RAW}" | grep -q "${SECRET_MSG}"; then
  echo "[-] Found plaintext on relay output!"; exit 1
fi

# Assert: required envelope fields exist
echo "${RAW}" | jq -e '.[0].cipher and (.[0].header|has("DHPub")) and (.[0].header|has("N"))' >/dev/null
